...
router.get("/itgov", (req, res, next) => {
  ...
  io.emit("get", { msg: "entrei na rota /itgov" });

  sql.connect(erro => {
    if (erro) {
      return res.status(501).send({ messagem: erro.message });
    }
...
router.post("/config", (req, res, next) => {
  ...
  sql.connect(erro => {
    if (erro) {
      return res.status(501).send({ error: erro.message });
    }

    sql
      .query(
        `SELECT * FROM TBL_CT_CONFIG_PREMISSAS WHERE ID_MES = '${req.body.id_mes}'`
      )
      .then(response => {
        if (response.recordset.length === 0) {
          sql
            .query(
              `INSERT INTO TBL_CT_CONFIG_PREMISSAS 
                    VALUES (
                        '${req.body.id_mes}',
...
router.get("/config/:ano", (req, res, next) => {
  ...
  sql.connect(erro => {
    ...
    sql
      .query(
        `SELECT * FROM TBL_CT_CONFIG_PREMISSAS WHERE ID_MES LIKE '%${ano}%' `
      )
      .then(dados => {
        const result = dados.recordset;
...
router.get("/previsaolanding", (req, res, next) => {
  sql.connect(erro => {
    if (erro) {
      return res.status(501).send({ erro: erro.message });
    }
...
router.post("/upload", upload.single("document"), (req, res, next) => {
  // eslint-disable-next-line no-undef
  io.emit("get", {
    photo: `${process.env.URL_PADRAO}uploads/${req.file.filename}`
  });
  res.status(200).send({
